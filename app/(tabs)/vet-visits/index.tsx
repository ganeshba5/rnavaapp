...
export default function VetVisitsScreen() {
  ...
  const { getVetVisitsByCanine, deleteVetVisit } = useApp();

  const [search, setSearch] = useState('');

  const visits = useMemo(() => {
    const entries = canineId ? getVetVisitsByCanine(canineId) : [];
    if (!search.trim()) return entries;
    const term = search.trim().toLowerCase();
    return entries.filter((visit) =>
      [visit.vetName, visit.reason, visit.endResults]
        .filter(Boolean)
        .some((value) => value.toLowerCase().includes(term))
    );
  }, [canineId, getVetVisitsByCanine, search]);
  ...
  const handleAdd = () => {
    ...
    router.push(`/(tabs)/vet-visits/create?canineId=${canineId}`);
  };

  const handleEdit = (visit: VetVisit) => {
    if (!canineId) return;
    router.push(`/(tabs)/vet-visits/create?canineId=${canineId}&visitId=${visit.id}`);
  };

  const confirmDelete = (visit: VetVisit) => {
    Alert.alert('Delete Visit', `Remove visit with ${visit.vetName}?`, [
  ...
  const openMenu = (visit: VetVisit) => {
    if (Platform.OS === 'ios') {
      ActionSheetIOS.showActionSheetWithOptions(
        ...
        (index) => {
          if (index === 1) handleEdit(visit);
          if (index === 2) confirmDelete(visit);
        }
      );
    } else {
      Alert.alert(visit.vetName, 'Choose an action', [
        { text: 'Edit', onPress: () => handleEdit(visit) },
        { text: 'Delete', style: 'destructive', onPress: () => confirmDelete(visit) },
        { text: 'Cancel', style: 'cancel' },
      ]);
    }
  };
    ...
    <ThemedView
      ...
      <View style={styles.headerRow}>
        ...
        <View style={styles.titleBlock}>
          <ThemedText style={[styles.title, { color: colors.text }]}>Vet Visits</ThemedText>
          <View style={[styles.countBadge, { backgroundColor: `${colors.tint}22` }]}
            accessibilityRole="text">
            <ThemedText style={[styles.countText, { color: colors.tint }]}>{visits.length}</ThemedText>
          </View>
        </View>
        <View style={{ width: 44 }} />
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={() => (
          <ThemedView style={[styles.emptyState, { borderColor: `${colors.icon}33` }]}
            ...
            <ThemedText style={[styles.emptyTitle, { color: colors.text }]}>No vet visits yet</ThemedText>
          ...
          <ThemedView
            ...
            <View style={styles.cardBody}>
              <View style={styles.fieldRow}>
                <ThemedText style={[styles.fieldLabel, { color: colors.secondaryText }]}>Date</ThemedText>
                <ThemedText style={[styles.fieldValue, { color: colors.text }]}>{formatDisplayDate(item.visitDate)}</ThemedText>
              </View>
              <View style={styles.fieldRow}>
                <ThemedText style={[styles.fieldLabel, { color: colors.secondaryText }]}>Reason</ThemedText>
                <ThemedText style={[styles.fieldValue, { color: colors.text }]}>{item.reason}</ThemedText>
              </View>
              <View style={styles.fieldRow}>
                <ThemedText style={[styles.fieldLabel, { color: colors.secondaryText }]}>End Results</ThemedText>
                <ThemedText style={[styles.fieldValue, { color: colors.text }]}>{item.endResults}</ThemedText>
              </View>
            </View>
